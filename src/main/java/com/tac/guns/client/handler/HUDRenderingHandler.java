    ...
    public void onOverlayRender(RenderGameOverlayEvent.Post event) {
        ...
        if(armorRig != null && armorRig.getItem() instanceof ArmorRigItem)
        {
            ...
            stack.pushPose();
            {
                ...
                RenderSystem.setShaderTexture(0,ARMOR_ICONS[1]);
                float durabilityPercentage = WearableHelper.currentDurabilityPercentage(armorRig);

                RenderSystem.setShaderColor(0.0f, 1.85f*durabilityPercentage, 0.0f, 1f);
                blit(stack, 0, 0, 0, 0, 16, 16, 16, 16);
                int cropHeight = (int) (16 * durabilityPercentage);

                RenderSystem.setShaderColor(1.0f, 1.0f, 1.0f, 1f);

                RenderSystem.setShaderColor(1.0f/durabilityPercentage, 0, 0.0f, 1f);
                RenderSystem.setShaderTexture(0,ARMOR_ICONS[0]);
